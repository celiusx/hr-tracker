...
function EmployeeCard({ emp, leaveTypes }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      ...
      {emp.allocations?.length === 0 && <p className="text-sm text-slate-400">No leave allocated.</p>}
...
export default function Dashboard() {
  ...
  const visibleEmployees = isEmployee
    ? employees.filter(emp => emp.id === currentUser?.employeeId)
    : employees

  const totalDaysUsed = visibleEmployees.reduce((sum, emp) =>
    sum + (emp.allocations?.reduce((s, a) => s + a.daysUsed, 0) || 0), 0)
  const totalDaysAllocated = visibleEmployees.reduce((sum, emp) =>
    ...
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">
          {isEmployee ? `My Leave Balance` : 'Dashboard'}
        </h1>
        <p className="text-sm text-slate-500 mt-1">
          {isEmployee ? 'Your leave allocation overview' : 'Overview of leave across all employees'}
        </p>
      </div>
      ...
      {visibleEmployees.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          {isEmployee
            ? 'No employee record linked to your account. Contact your HR.'
            : 'No employees yet. Go to Employees to add some.'}
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {visibleEmployees.map(emp => (
            <EmployeeCard key={emp.id} emp={emp} leaveTypes={leaveTypes} />
          ))}
        </div>
      )}
    </div>
